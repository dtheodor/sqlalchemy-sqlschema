...
class TestPostgresSqlStatementExecution(object):
    ...
    def test_get_schema(self, pg_session, pg_test_schema):
        ...
        pg_session.execute("set search_path to new_schema")
    ...
    def test_set_schema(self, pg_session):
        pg_session.execute(set_schema("another_schema"))
        assert pg_session.execute("show search_path").scalar() == \
               "another_schema"

        pg_session.execute(set_schema("another_schema,public"))
        assert pg_session.execute("show search_path").scalar() == \
...
class TestSessionMaintainSchemaPostgres(object):
    ...
    def test_maintain_schema(self, pg_session, pg_test_schema):
        ...
        m = maintain_schema("schema2,public", pg_session)
...
class TestMaintainSchemaNestedPostgres(object):
    ...
    def test_maintain_schema_nested(self, pg_session, pg_test_schema):
        ...
        m_level1 = maintain_schema("schema_level1,public", pg_session)
        m_level2 = maintain_schema("schema_level2,public", pg_session)
        with mock.patch.object(
            ...
            with m_level1:
                self.assert_the_schema_abides(
                    m_level1, pg_session, 'schema_level1, public')

                with m_level2:
                    self.assert_the_schema_abides(
                        m_level2, pg_session, 'schema_level2, public')

                # must be reverted to level1
                self.assert_the_schema_abides(
                    m_level1, pg_session, 'schema_level1, public', called_so_far=2)
